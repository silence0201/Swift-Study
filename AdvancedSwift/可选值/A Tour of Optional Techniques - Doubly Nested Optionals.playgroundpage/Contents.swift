...
integer. Here, the last entry will be a `nil`, since `"foo"` isn't an integer.
...
/*:
When the loop gets to the final element — the `nil` from "foo" — what's returned
from `next` is a non-`nil` value: `.some(nil)`. It unwraps this and binds what's
inside (a `nil`) to `maybeInt`. Without doubly wrapped optionals, this wouldn't
be possible.
