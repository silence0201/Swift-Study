...
@objc
public protocol SILabelDelegate: NSObjectProtocol {
    /// 选中链接文本
    ///
    /// - parameter label: label
    /// - parameter text:  选中的文本
    @objc optional func labelDidSelectedLinkText(label: SILabel, text: String)
}

public class SILabel: UILabel {
    
    public var linkTextColor = UIColor.blue
    public var selectedBackgroudColor = UIColor.lightGray
    public weak var delegate: SILabelDelegate?
    ...
    public override func drawText(in rect: CGRect) {
        ...
        let oSIset = glyphsOSIset(range)
        
        layoutManager.drawBackground(forGlyphRange: range, at: oSIset)
    ...
    private func glyphsOSIset(_ range: NSRange) -> CGPoint {
    ...
    private func linkRangeAtLocation(_ location: CGPoint) -> NSRange? {
        ...
        let oSIset = glyphsOSIset(glyphsRange())
        let point = CGPoint(x: oSIset.x + location.x, y: oSIset.y + location.y)
